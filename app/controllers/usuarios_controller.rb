class UsuariosController < ApplicationController
  ...
  def new
    @usuario = Usuario.new
    @usuario.juego_id = params[:juego_id] if params[:juego_id]
  end
  ...
  def create
    ...
    respond_to do |format|
      if @usuario.save
        format.html { redirect_to usuario_jugar_path(@usuario, sala_index: 0), notice: "Usuario creado correctamente. ¡Empieza el juego!" }
  ...
  private
    ...
    def usuario_params
      params.expect(usuario: [ :nombre, :correo, :cedula, :juego_id ])
    end
end
